...
const Evenements = memo(() => {
  ...
  const { currentUser, isLoadingAuth, currentUserAirtableData } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!currentUser?.email || !airtableRecordId) {
    ...
    const evenementData: any = { // Utilisation de any pour l'instant
        ...
        'Nombre de personnes': parseInt(formData.nombrePersonnes),
  ...
  const getDateLocale = (): Locale => i18n.language.startsWith('th') ? th : i18n.language.startsWith('en') ? enUS : fr;
  
  const isLoading = isLoadingAuth || (currentUser && dataIsLoading);
                  ...
                  <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-4 p-2">
                    {plats.map((plat: Plat) => (
                      <div key={plat.id} className="flex items-center space-x-3">
                        <Checkbox id={`plat-event-${plat.id}`} checked={platsPreSelectionnes.includes(plat.id)} onCheckedChange={(checked) => handlePlatSelectionChange(plat.id, !!checked)} />
                        <Tooltip>
                          ...
                          <TooltipContent className="w-64 bg-white border-thai-orange p-2 rounded-md shadow-lg">
                            {plat['Photo du Plat'] && (<img src={plat['Photo du Plat'][0].url} alt={plat.Plat} className="w-full h-32 object-cover rounded-md mb-2" />)}

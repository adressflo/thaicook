...
import { useAuth } from '@/contexts/AuthContext'; // AJOUT: Importer useAuth
...
const Sidebar = () => {
  ...
  const { currentUserRole, isLoadingUserRole, currentUser } = useAuth(); // AJOUT: Récupérer le rôle et l'état de chargement
  ...
  // Ne pas afficher le lien admin si le rôle est en cours de chargement ou si l'utilisateur n'est pas admin
  const showAdminLink = currentUser && !isLoadingUserRole && currentUserRole === 'admin'; //
    ...
    <>
      ...
      <div className={cn(
        ...
        <nav className="flex-grow p-3 space-y-1 overflow-y-auto">
          ...
          {showAdminLink && ( // AJOUT: Condition pour afficher le lien Admin
            <div className="pt-4 mt-4 border-t border-thai-orange/10">
              ...
              >
                <Shield className={cn("h-5 w-5 flex-shrink-0", isAdminActive('/admin') ? "text-white" : "text-thai-green/80 group-hover:text-thai-green")} />

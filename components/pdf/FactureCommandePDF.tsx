...
import type { commande_db, details_commande_db, client_db } from '@/generated/prisma/client';
  ...
  <Document>
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.section}>
        ...
        <Text style={styles.text}>Date: {new Date(commande.date_de_prise_de_commande || "").toLocaleDateString('fr-FR')}</Text>
        <Text style={styles.text}>Client: {commande.client_db?.prenom} {commande.client_db?.nom}</Text>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.table}>
          ...
          <View style={styles.tableRow}>
            <View style={styles.tableColHeader}><Text style={styles.tableHeaderText}>Plat</Text></View>
            <View style={styles.tableColHeader}><Text style={styles.tableHeaderText}>Quantité</Text></View>
            <View style={styles.tableColHeader}><Text style={styles.tableHeaderText}>Prix Unitaire</Text></View>
            <View style={styles.tableColHeader}><Text style={styles.tableHeaderText}>Total</Text></View>
          </View>
          {/* Table Body */}
          {(commande.details || []).map(detail => (
            <View style={styles.tableRow} key={detail.iddetails}>
              <View style={styles.tableCol}><Text style={styles.tableCellText}>{detail.nom_plat}</Text></View>
              <View style={styles.tableCol}><Text style={styles.tableCellText}>{detail.quantite_plat_commande}</Text></View>
              <View style={styles.tableCol}><Text style={styles.tableCellText}>{Number(detail.prix_unitaire).toFixed(2)} €</Text></View>
              <View style={styles.tableCol}><Text style={styles.tableCellText}>{(Number(detail.prix_unitaire) * (detail.quantite_plat_commande || 1)).toFixed(2)} €</Text></View>
            </View>
          ))}
        </View>
      </View>
      ...
      <View style={{...styles.section, marginTop: 20, textAlign: 'right'}}>
         <Text style={{...styles.text, fontSize: 12, fontWeight: 'bold'}}>
            Total: {(commande.details || []).reduce((acc, detail) => acc + (Number(detail.prix_unitaire) * (detail.quantite_plat_commande || 1)), 0).toFixed(2)} €
        </Text>
      </View>

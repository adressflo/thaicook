    ...
    queryFn: async (): Promise<PlatRupture[]> => {
      ...
      if (error) {
        const contextError = new Error(
          `Échec chargement ruptures plat (${platId}): ${error.message || "Erreur base de données"}`
        )
...
export const useCreatePlatRupture = () => {
  ...
  return useMutation({
    mutationFn: async (ruptureData: {
      ...
      if (error) {
        const contextError = new Error(
          `Échec création rupture: ${error.message || "Erreur base de données"}`
        )
        contextError.cause = error
        throw contextError
      }
...
export const useDeletePlatRupture = () => {
  ...
  return useMutation({
    mutationFn: async (ruptureId: number) => {
      ...
      if (error) {
        const contextError = new Error(
          `Échec suppression rupture: ${error.message || "Erreur base de données"}`
        )
        contextError.cause = error
        throw contextError
      }
